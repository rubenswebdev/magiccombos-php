...
class importador extends Drk{
    ...
    function getList(){
            ...
            if(!$this->edicoes->ver_se_existe($edicao)){
    ...
    public function get_links(){
        $this->url_all = 'http://magiccards.info/sitemap.html';
        $html = file_get_html($this->url_all);
        $edicoes_html = $html->find('body > a');
        for($i = 0;$i <= 12;$i++){ //elimina os links que nao sao edicoes
            unset($edicoes_html[$i]);
        }
        foreach($edicoes_html as $e){
            if(strpos($e->href, '.html') ){
                $urls[] = 'http://magiccards.info'.$e->href;
            }
        }
    ...
    public function simu(){
            ...
            $this->url = 'http://magiccards.info/'.$this->segments(4).'/'.$this->segments(5).'.html';
